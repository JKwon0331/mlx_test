...
from layer import Swish, GLU, DepthwiseConv1d, PointwiseConv1d, MultiHeadAttention, recover_resolution, TimeReductionLayer_2d

class Model(nn.Module):

    def __init__(self, d_input, d_output: int = 80, d_layers = 1,e_layers = 4, d_model: int = 512, conv_expansion_factor: int = 2,
          ff_dropout: float = 0.1, nhead = 20, conv_dropout: float = 0.1, kernel_size: int = 31, ff_expansion_factor: int = 4,
        ...
        self.enc=SqueezeformerEncoder( encoder_dim=d_model, num_layers=e_layers,
        ...
        if not (d_layers == 0):
            self.rnn = nn.LSTM(d_model, d_model, d_layers)
    ...
    def __init__(
        ...
        for idx in range(num_layers):
            if idx < reduce_layer_index:
                self.layers['layer' + str(idx)] = SqueezeformerBlock(
                ...
                self.layers['layer' + str(idx)] = ResidualConnectionModule(
    ...
    def __call__(self, inputs):
        ...
        for idx,layer in enumerate(self.layers):
            ...
            if idx == self.recover_layer_index:
                ...
                outputs += recover_tensor
            
            outputs = self.layers['layer' + str(idx)](outputs)
...
class ConvModule(nn.Module): 
    ...
    def __init__(
        ...
        self.sequential = nn.Sequential(
            ...
            GLU(dim=2),
            DepthwiseConv1d(out_channels, out_channels, kernel_size, stride=1, padding=((kernel_size - 1) // 2, (kernel_size - 1) // 2)),
            nn.BatchNorm(out_channels),#, track_running_stats = False),
            Swish(),
...
class MultiHeadedSelfAttentionModule(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, dropout_p: float = 0.1):  ##emb = 1: RoPE, 2: Relative Position Encoding
